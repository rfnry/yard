...
| **`output_schemas`** | The engine wires Pydantic classes to task names (`output_schemas=OutputSchemas(tasks={"market-scan": MarketScan, ...})`); the harness validates each reply via Pydantic before returning. |
