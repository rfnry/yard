...
- **min_eval_cases: 3** — `Agent.turn` raises `ReflectionError` until at least 3 eval cases are extracted (use `Agent.extract_eval_case` from low-quality outcomes).
- **auto_consolidation.threshold: 5** — auto-consolidates after every 5 outcomes, with a 1-hour cooldown.

Calibration verdicts of `overconfident` or `uncorrelated` automatically suppress promotion (existing v3 behavior) and additionally fire `SelfEvolvingCalibrationAlertEvent` so the host can notice. Promoted learnings flow straight into `learned/_index.md` and the next boot bundle.
